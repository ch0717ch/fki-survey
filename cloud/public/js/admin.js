...
function handleAction(action, btn) {
  ...
  switch (action) {
    ...
    case 'export':
      // Workers에서는 exceljs가 못 돌아가므로 응답만 받아 브라우저에서 만든다.
      toast('엑셀을 만드는 중입니다…');
      return api(`/api/admin/surveys/${survey.id}/responses`)
        .then(({ responses }) => window.downloadSurveyXlsx(survey, responses))
        .then((fileName) => toast(`${fileName} 저장됨`))
